...
def _run_prefetch(decision, query: str, combined: str, q_vec: list) -> str:
    ...
    if len(routes_to_run) == 1:
        return _fetch_single_route(
            routes_to_run[0], temporal_range, query, combined, q_vec
        )
...
def _prepare_turn(user_message: str, conversation_id: str):
    ...
    try:
        q_vec = gateway.embed(
            combined, embed_model=EMBED_MODEL, priority=Priority.INTERACTIVE
        )
    except Exception:
        pass
...
def run_stream(user_message: str, conversation_id: str):
    ...
    messages, active_tools, prefetch_active, user_message = _prepare_turn(
