...
MEMORY_MIN_SIM = (
    0.55  # floor — below this a fact is unrelated (was 0.30; raised to reduce noise)
)
...
_PROFILE_MIN_SIM = (
    0.25  # lower than fact threshold — fields are shorter and more general
)
...
def delete_note(note_text: str) -> str:
    ...
    for fact_id in matched:
        _conn.execute(
            "UPDATE memory_facts SET valid_to = ? WHERE fact_id = ?", (now, fact_id)
        )
    _conn.commit()
