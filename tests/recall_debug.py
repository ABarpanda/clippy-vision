...
for label, keywords, query_str in probes:
    ...
    print(f"--- {label} ---  fts_q={fts_q}  range={time.strftime('%m-%d', time.localtime(tr.start_ts))} to {time.strftime('%m-%d', time.localtime(tr.end_ts))}")

    for name, sql in [("alias", sql_alias), ("no-alias", sql_noalias)]:
        try:
            rows = conn.execute(sql, (fts_q, tr.start_ts, tr.end_ts)).fetchall()
            if rows:
                print(f"  [{name}] {len(rows)} rows, top rank={rows[0][3]:.3f}, title={rows[0][2][:50]}")
            else:
                print(f"  [{name}] 0 rows")
